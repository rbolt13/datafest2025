...
There are three general-purpose lexicons that are included with tidytext. We will be using two. 
...
These dictionaries were constructed via either crowdsourcing (using, for example, Amazon Mechanical Turk) or by the labor of one of the authors, and were validated using some combination of crowdsourcing again, restaurant or movie reviews, or Twitter data. Given this information, we may hesitate to apply these sentiment lexicons to styles of text dramatically different from what they were validated on. [^3]

[^3]: [Text Mining with R - Sentiment analysis with tidy data](https://www.tidytextmining.com/sentiment#the-sentiments-datasets)
...
song_sent <- unigram |>
  ...
  group_by(album, track.1) |>
  summarise(score = mean(value), .groups = "drop") |>
  arrange(album, track.1)

ggplot(song_sent, aes(track.1, score)) +
  geom_col() +
  facet_wrap(~ album, scales = "free_x") +
  labs(y = NULL)
```

## Album Sentiments Overtime
...
bigram |>
  ...
  ungroup() |>
  mutate(bigram = tidytext::reorder_within(bigram, n, sentiment)) |>
  ggplot(aes(n, bigram, fill = sentiment)) +
  geom_col(show.legend = FALSE) +
...
base <- Data |>
  ...
  count(album, track.1, sentiment, name = "n") |>
  pivot_wider(names_from = sentiment, values_from = n, values_fill = 0) |>
  mutate(net_before = positive - negative)

# 2) Negated sentiment counts per track (from bigrams)
negflip <- Data |>
  unnest_tokens(bigram, lyrics, token = "ngrams", n = 2) |>
  separate(bigram, c("w1","w2"), sep = " ", remove = FALSE) |>
  inner_join(bing, by = c("w2" = "word")) |>
  filter(w1 %in% negations$word) |>
  count(album, track.1, sentiment, name = "k") |>
  pivot_wider(names_from = sentiment, values_from = k, values_fill = 0, names_prefix = "neg_")

# 3) Adjust per track and compute after/before
by_track <- base |>
  left_join(negflip, by = c("album","track.1")) |>
  mutate(across(starts_with("neg_"), ~ replace_na(.x, 0))) |>
  mutate(
    pos_after = positive - neg_positive + neg_negative,
    neg_after = negative - neg_negative + neg_positive,
    net_after = pos_after - neg_after
  )

# 4) Album-level: track averages and the change (after - before)
album_delta <- by_track |>
  group_by(album) |>
  summarise(
    track_avg_before = mean(net_before),
    track_avg_after  = mean(net_after),
    delta            = track_avg_after - track_avg_before,
    .groups = "drop"
  )

# (A) Quick “how much did it change?” bar
ggplot(album_delta, aes(delta, album)) +
  geom_col() +
  labs(x = "Δ net sentiment (after - before)", y = NULL)

```

## Word Cloud

A ackage to create word clouds and visualize frequency. 
...
A set of curated color palettes (qualitative, sequential, diverging) designed for readable data viz and maps, many color-blind friendly.
...
A legacy R package for reshaping data between wide and long formats.
...
1. How do Bing and AFINN differ in how they score words?

2. Why do we sometimes use inner_join() vs left_join() when scoring sentiment?

3. When comparing albums, why normalize by length (e.g., use a rate) instead of raw counts?

4. In bigrams, why flip sentiment based on w1 matching a negation and not w2?

5. When is a word cloud appropriate, and when is a bar chart better?
